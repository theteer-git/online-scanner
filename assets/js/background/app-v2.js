...
const state={
  sourceFile:null,sourceBitmap:null,rawCutout:null,mask:null,segmenter:null,bgBitmap:null,
...
async function loadImage(file){
  ...
  state.sourceBitmap?.close?.();state.rawCutout?.close?.();
  state.sourceFile=file;
  state.sourceBitmap=await createImageBitmap(file);
  state.rawCutout=null;state.mask=null;state.history=[];state.future=[];
...
function resetApp(){
  state.sourceBitmap?.close?.();state.rawCutout?.close?.();state.bgBitmap?.close?.();
...
window.addEventListener("beforeunload",()=>{state.sourceBitmap?.close?.();state.rawCutout?.close?.();state.bgBitmap?.close?.()});
